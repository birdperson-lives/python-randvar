...
class InviableRandomVariableError(Exception):
    """
    Exception raised when floating-point rounding has degraded values beyond use.
...
def rand_apply(func, *args, viability=1.0, **kwargs):
    """
    Applies a function to random variable arguments, returning a random variable representing the distribution of 
    return values from the function.

    All non-random variable arguments to the function are treated as constant distributions.

    The viability of the returned variable is the minimum of the viabilities of the arguments and the viability 
    ...
    for arg in args:
        if isinstance(arg, RandomVariable):
            ...
            rand_args.append(RandomVariable({arg: 1.0}))
    rand_kwargs = {}
    for name in kwargs:
        if isinstance(kwargs[name], RandomVariable):
            rand_kwargs[name] = kwargs[name]
        else:
            rand_kwargs[name] = RandomVariable({kwargs[name]: 1.0})
...
def randomable(func, viability=1.0):
    """
    A function wrapper so that the functions returns a random variable representing the distribution of return values of
    the body given that all the arguments are random variables.

    All non-random variable arguments to the function are treated as having constant distributions.
