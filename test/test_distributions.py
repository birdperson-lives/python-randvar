...
class TestDistribuitons(unittest.TestCase):
    ...
    def test_const(self):
        """
        Tests the `const` distribution by ensuring that only one value is ever
        sampled.
        """
    ...
    def test_uniform(self):
        """
        Tests the `uniform` distribution by ensuring that all values are
    ...
    def test_poisson_trunc(self):
        """
        Tests the `poisson_trunc` distribution by comparing to the known
        distribution.
        """
        ...
        for n in range(0, 10):
            ...
            for k in range(n):
                self.assertTrue(math.isclose(poissonvar[k],
                                             expect ** k / math.factorial(
                                                 k) * math.exp(-expect),
                                             rel_tol=1e-05, abs_tol=1.0))

    def test_poisson_stretch(self):
        """
        Tests the `poisson_stretch` distribution by comparing to the known
        distribution.
        """
